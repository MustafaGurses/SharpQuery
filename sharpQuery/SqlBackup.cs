...
namespace sharpQuery
{
    class SqlBackup : IDisposable
    {
        ...
        const string getDBSize = "USE [{0}] SELECT CAST(SUM(size) * 8. / 1024 AS BIGINT) FROM sys.master_files WITH(NOWAIT) WHERE database_id = DB_ID() GROUP BY database_id";
        const string createBakFiles1 = "BACKUP DATABASE [{0}] TO DISK = N'{0}_tmp_1.bak'";
        const string createBakFiles2 = " ,DISK = N'{0}_tmp_{1}.bak'";
        const string createBakFiles3 = " WITH NOFORMAT, NOINIT,  NAME = N'{0}-Full Database Backup', SKIP, NOREWIND, NOUNLOAD,  STATS = 10";
        ...
        {
            string script = string.Format(createBakFiles1, sqlConnection.Database);
            ...
            {
                script += string.Format(createBakFiles2, sqlConnection.Database, i);
            }

            script += string.Format(createBakFiles3, sqlConnection.Database);
        ...
        {
            sqlCmd = new SqlCommand(string.Format(insertBakFile, tmpDBName, string.Format(bakFileName, this.defaultBakPath, sqlConnection.Database, i)), sqlConnection);
            sqlCmd.CommandTimeout = sqlConnection.ConnectionTimeout;
            sqlCmd.ExecuteNonQuery();
